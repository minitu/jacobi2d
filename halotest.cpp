...
int main(int argc, char** argv) {
  ...
  for (int iter = 1; iter <= n_iters; iter++) {
    ...
    if (rank == 0) {
      for (int j = 0; j < world_size; j++) {
        ...
        printf("[%03d,%03d] Pack: %.3lf, Barrier: %.3lf, Halo: %.3lf, Iter: %.3lf\n",
            iter, j,
            global_durations[N_DUR*j] * 1000000, global_durations[N_DUR*j+1] * 1000000,
            global_durations[N_DUR*j+2] * 1000000, iter_time * 1000000);
      }
    }
    */
#endif
  ...
  if (rank == 0) {
    for (int j = 0; j < world_size; j++) {
      ...
      printf("[average,%03d] Pack: %.3lf, Barrier: %.3lf, Halo: %.3lf, Iter: %.3lf\n", j,
          global_durations[N_DUR*j] * 1000000, global_durations[N_DUR*j+1] * 1000000,
          global_durations[N_DUR*j+2] * 1000000, total_time * 1000000);
    }
  }
  ...
  if (rank == 0) {
    for (int j = 0; j < world_size; j++) {
      ...
      printf("[minimum,%03d] Pack: %.3lf, Barrier: %.3lf, Halo: %.3lf, Iter: %.3lf\n", j,
          global_durations[N_DUR*j] * 1000000, global_durations[N_DUR*j+1] * 1000000,
          global_durations[N_DUR*j+2] * 1000000, total_time * 1000000);
    }
  }
  ...
  if (rank == 0) {
    double halo_max = 0;
    for (int j = 0; j < world_size; j++) {
      if (global_durations[N_DUR*j+2] > halo_max) {
        halo_max = global_durations[N_DUR*j+2];
      }
    }

    printf("[final] Max Halo: %.3lf\n", halo_max * 1000000);
  }
#endif
